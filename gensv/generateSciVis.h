...
namespace gensv {
  ...
  struct CosmicWeb {
    std::vector<ospray::cpp::Geometry> bricks;
    box3f localBounds;
  };

  // The grid passed sets what subset of the original data should be loaded,
  // from a single brick (1^3) to the entire dataset (8^3)
  CosmicWeb loadCosmicWeb(const std::string &basePath, const vec3i &cosmicWebGrid);

  struct LoadedVolume {
    ospray::cpp::Volume volume;
    std::vector<ospray::cpp::Geometry> isosurfaces;
    // Kind of a hack to color the isosurfaces consistently when doing hybrid-parallel
    std::vector<size_t> isosurfaceValueIndex;
